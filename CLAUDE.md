...
- **Backend — migrating to FastAPI Cloud (from Railway).** As of 2026-08, the live production `NEXT_PUBLIC_API_URL` actually pointed at a Railway deployment (`*.up.railway.app`), not Render — Railway was the real prod backend despite `render.yaml` describing Render. No Railway config file lives in this repo (deployment was set up directly in the Railway dashboard/CLI). Migrating prod from Railway to FastAPI Cloud.
  - `backend/pyproject.toml` sets `[tool.fastapi] entrypoint = "app.main:app"` since the app object lives at `app/main.py`, not project-root `main.py`. `backend/.python-version` pins `3.11.9`. Deploy from inside `backend/` with `fastapi deploy` (installs `fastapi[standard]`, which bundles the `fastapi-cloud-cli`).
  - `ANTHROPIC_API_KEY` and `ALLOWED_ORIGINS` are set as FastAPI Cloud environment variables/secrets (dashboard or `fastapi env`), not committed. `ALLOWED_ORIGINS` must include the production Vercel origin above.
  - After deploying, `NEXT_PUBLIC_API_URL` on Vercel must be updated to the `https://<app>.fastapicloud.dev` URL FastAPI Cloud assigns, replacing the Railway URL.
- **Fallback: Render.** `render.yaml` (repo root) deploys the backend as a Render web service: `pip install -r backend/requirements.txt`, `cd backend && uvicorn app.main:app --host 0.0.0.0 --port $PORT`. `ANTHROPIC_API_KEY` and `ALLOWED_ORIGINS` are set in the Render dashboard (`sync: false`). Repo-root `.python-version` pins `3.11.9` for Render compatibility. Never actually served production traffic (Railway did); kept only as a rollback option.
